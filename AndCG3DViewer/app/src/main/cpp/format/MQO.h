...
public:
	...
	int         visible;						/* 可視/不可視 */
    int         shading;                        /* 法線適用種別 */
    CG3DVector4f  color;                          /* カラー */
    std::vector<CG3DVector3f>     Vertex;         /* 頂点データ */
    std::vector<CG3DVector3f>     Normal;         /* 法線データ */
...
private:
    ...
    static void LoadVertex(std::vector<std::string> &contents, unsigned int &lpct, std::vector<CG3DVector3f> &aVertexs);
    ...
    static CG3DVector3f calcNormal(CG3DVector3f &aV0, CG3DVector3f &aV1, CG3DVector3f &aV2);
    static float *vector2array(std::vector<CG3DVector3f> &vector);
